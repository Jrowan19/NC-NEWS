...
describe('makeRefObj', () => {
  ...
  it('given multiple objects within an array, a lookup object should be returned', () => {
    const input = [{ name: 'john', age: 35 }, { name: 'julie', age: 34 }, { name: 'paul', age: 33 }]
...
describe('formatComments', () => {
  ...
  it('should rename the created_by key to author', () => {
    const input = [{ created_by: 'butter_bridge', created_at: 1479818163389, belongs_to: 'Living in the shadow of a great man' }];
  ...
  it('converts the keys for multiple array objects passed', () => {
    ...
    const actual = formatComments(input, lookup);
    const expected = [{ author: 'icellusedkars', created_at: new Date(1385210163389), article_id: 1, body: 'I hate streaming noses', votes: 0 },
    { author: 'paul', created_at: new Date(1353674163389), article_id: 2, body: 'I hate streaming eyes even more', votes: 0 }];
    expect(actual).to.eql(expected);
  });
});
